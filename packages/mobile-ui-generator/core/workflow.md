...
4. **视觉设计与迭代确认**：
   - **`.pen` 文件处理**：若用户提供了 `.pen` 文件，使用 `open_document` 打开该文件；若用户未提供 `.pen` 文件，使用 `open_document` 传入 `'new'` 创建新的 `.pen` 文件。禁止因缺少 `.pen` 文件而跳过设计步骤。
...
当 AI 进入计划模式时，计划中 **必须** 包含完整的 Pencil 设计流程步骤，不得省略或合并：

1. Pencil 安装/验证步骤
2. 输出至少 3 个差异化 UI 方案并等待用户选择
3. 调用 Pencil 生成设计稿
4. 展示设计稿并等待用户明确确认
5. 确认后才安排代码实现

**计划模式不是绕过设计流的理由。** 绝对禁止在计划中跳过 Pencil 设计步骤直接安排代码实现。
